...
MAIN_SRC = $(SRC_DIR)/formula.c
TEST_SRC = $(TEST_DIR)/tests.c

# Объектные файлы
OBJS = $(BUILD_DIR)/formula.o
MAIN_OBJ = $(BUILD_DIR)/formula_main.o
...
$(TARGET): $(MAIN_OBJ)
	$(CC) $(CFLAGS) -o $(TARGET) $(MAIN_OBJ) $(MATH_FLAG)
...
$(TEST_OBJ): $(TEST_SRC) $(INCLUDE_DIR)/formula.h
	$(CC) $(CFLAGS) -I$(INCLUDE_DIR) -c $(TEST_SRC) -o $(TEST_OBJ)

# Компиляция основного объектного файла
